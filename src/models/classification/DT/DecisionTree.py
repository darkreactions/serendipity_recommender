...
from BaseClassifier import ActiveLearningClassifier
...
def run_model(DecisionTree_params, category):
    ...
    features = [
        ...
        "_stoich_inorg/solv",
        "_stoich_acid/solv",
        "_stoich_org+inorg/solv",
        "_stoich_org+inorg+acid/solv",
        "_stoich_org/liq",
        "_stoich_inorg/liq",
        "_stoich_org+inorg/liq",
        "_stoich_org/inorg",
        "_stoich_acid/inorg",
        "_rxn_Temperature_C",
        "_rxn_Reactiontime_s",
    ...
    config = (
        DecisionTree_params["configs"][category]
        if DecisionTree_params["configs"]
        else None
    )
    ...
    if fine_tuning:
        class_weights = [{0: i, 1: 1.0 - i} for i in np.linspace(0.05, 0.95, num=50)]
        ...
        for amine in amine_list:
            ...
            for set_id in draws:
                ...
                x_t, y_t, x_v, y_v, all_data, all_labels = (
                    dataset[set_id]["x_t"],
                    dataset[set_id]["y_t"],
                    dataset[set_id]["x_v"],
                    dataset[set_id]["y_v"],
