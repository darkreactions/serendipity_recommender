...
# using 10 previous active learning steps
k = 11

# get top 9 predictions
n_predict = 9
...
pred_order = order(bart_yhat_mean, decreasing = T)

dat_stateset_ordered = dat_stateset[pred_order, ]
dat_stateset_ordered$success_prob = bart_yhat_mean[pred_order]

model_name = paste("CLB", model, "_predictions_all.csv", sep = "")

write.csv(dat_stateset_ordered, file = paste("data/", new_amine, "/", model_name, sep = ""), row.names = F)

model_name = paste("CLB", model, "_predictions.csv", sep = "")

write.csv(dat_stateset_ordered[1:n_predict, ], file = paste("data/", new_amine, "/", model_name, sep = ""), row.names = F)
...
rownames(reagents) = as.character(reagents$name)

reagents_ordered = reagents[as.character(dat_stateset_ordered$name), ]

model_name = paste("CLB", model, "_reagents.csv", sep = "")
write.csv(reagents_ordered[1:n_predict, ], file = paste("data/", new_amine, "/", model_name, sep = ""), row.names = F)
